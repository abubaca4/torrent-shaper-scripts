...
# incoming traffic
tc qdisc add dev $WAN_IFB root handle 1: htb default 20
tc class add dev $WAN_IFB parent 1: classid 1:1 htb rate $DL_SPEED ceil $DL_SPEED quantum 1500
tc class add dev $WAN_IFB parent 1:1 classid 1:10 htb rate 10Mbit ceil $DL_SPEED prio 2 quantum 1500
tc class add dev $WAN_IFB parent 1:1 classid 1:20 htb rate 2Mbit ceil $DL_SPEED prio 3 quantum 1500
tc class add dev $WAN_IFB parent 1:1 classid 1:30 htb rate 2Mbit ceil $DL_SPEED prio 4 quantum 1500
tc qdisc add dev $WAN_IFB parent 1:10 handle 10: fq_codel limit 10240 quantum 300
tc qdisc add dev $WAN_IFB parent 1:20 handle 20: fq_codel limit 10240 quantum 300
tc qdisc add dev $WAN_IFB parent 1:30 handle 30: pfifo limit 10240
...
## incoming
#ICMP
tc filter add dev $WAN_IFB parent 1: protocol ip prio 1 u32 match ip protocol 1 0xff flowid 1:10
#ACK
tc filter add dev $WAN_IFB parent 1: protocol ip prio 1 u32 match ip protocol 6 0xff match u8 0x05 0x0f at 0 match u16 0x0000 0xffc0 at 2 match u8 0x10 0xff at 33 flowid 1:10
#DNS
tc filter add dev $WAN_IFB parent 1: protocol ip prio 1 u32 match ip protocol 17 0xff match ip sport 53 0xffff flowid 1:10
#VOIP
tc filter add dev $WAN_IFB parent 1: protocol ip prio 2 u32 match ip tos 0x68 0xff match ip protocol 11 0xff flowid 1:10
tc filter add dev $WAN_IFB parent 1: protocol ip prio 2 u32 match ip tos 0xb8 0xff match ip protocol 11 0xff flowid 1:10
#TOS
tc filter add dev $WAN_IFB parent 1: protocol ip prio 2 u32 match ip tos 0x10 0xff flowid 1:10
#IPTV
#tc filter add dev $WAN_IFB parent 1: protocol ip prio 2 u32 match ip protocol 17 0xff match ip dst 224.0.0.0/3 flowid 1:10
#NTP
tc filter add dev $WAN_IFB parent 1: protocol ip prio 2 u32 match ip protocol 17 0xff match ip sport 123 0xffff flowid 1:10
...
if [ "$enable_mac_filter" = true ] ; then
    ...
    do
        ...
        flowid 1:30
        if [ "$enable_ipv6_torrent" = true ] ; then
            tc filter add dev $WAN_IFB parent 1: protocol ipv6 prio 8 u32 $incoming_mac_filter\
            match ip dport $temp_port 0xffff \
            flowid 1:30
        fi
        ## end incoming
